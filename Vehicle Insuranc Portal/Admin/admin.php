    ...
    <div class="navcontainer">
		...
		<div class="main">
			...
			<div class="box box1">
				...
				function daysUntilExpiry1($dueDate) {
				...
				while ($row = $query->fetch_assoc()) {
					$daysDiffPUC = daysUntilExpiry1($row['puc_due']);
					...
					<div class="table-responsive">
						<table id="myTable" class="table table-bordered table-striped">
							...
							while ($row = $query->fetch_assoc()) {
								...
								echo "<tr>
									<td>" . $row['id'] . "</td>
									<td" . (($daysDiff <= 15 && $daysDiff >= 0) ? " style='color:#ff5349 ; font-weight:bold'" : "") . ">" . $row['registration_no'] . "</td>
									<td" . (($daysDiff <= 15 && $daysDiff >= 0) ? " style='color:#ff5349 ; font-weight:bold'" : "") . ">" . $row['vehicle_class'] . "</td>
									<td" . (($daysDiff <= 15 && $daysDiff >= 0) ? " style='color:#ff5349 ; font-weight:bold'" : "") . ">" . $row['body_type'] . "</td>
									<td" . (($daysDiff <= 15 && $daysDiff >= 0) ? " style='color:#ff5349 ; font-weight:bold'" : "") . ">" . $row['insurance_due'] . "</td>
									<td>
			...
			<div class="height10"></div>
					...
					<div class="table-responsive">
						<table id="myTable1" class="table table-bordered table-striped">
							...
							function dateDiffInDays1($puc_due) {
								$currentTimestamp = strtotime('today');
								$expiryTimestamp = strtotime($puc_due);
								$diff = ($expiryTimestamp - $currentTimestamp) / (60 * 60 * 24);
								return floor($diff);
							}
							...
							while ($row = $query->fetch_assoc()) {
								$daysDiff = dateDiffInDays1($row['puc_due']);
								...
								echo "<tr>
									...
									<td" . (($daysDiff <= 15 && $daysDiff >= 0) ? " style='color:#ff5349 ; font-weight:bold'" : "") . ">" . $row['vehicle_class'] . "</td>
									<td" . (($daysDiff <= 15 && $daysDiff >= 0) ? " style='color:#ff5349 ; font-weight:bold'" : "") . ">" . $row['body_type'] . "</td>
									<td" . (($daysDiff <= 15 && $daysDiff >= 0) ? " style='color:#ff5349 ; font-weight:bold'" : "") . ">" . $row['puc_due'] . "</td>
									<td>
										<a href='#edit_" . $row['id'] . "' class='btn btn-success btn-sm' data-toggle='modal'><i class='fas fa-eye'></i>  View</a>
										<a href='#delete_" . $row['id'] . "' class='btn btn-danger btn-sm' data-toggle='modal'><span class='glyphicon glyphicon-trash'></span> Delete</a>
	...
	<div class="content" id="content-tab3" style="display: none;">
			...
			<div class="height10"></div>
					...
					<div class="table-responsive">
                        ...
                        function dateDiffInDays2($fitness_due) {
                            $currentTimestamp = strtotime('today');
                            $expiryTimestamp = strtotime($fitness_due);
                            $diff = ($expiryTimestamp - $currentTimestamp) / (60 * 60 * 24);
                            return floor($diff);
                        }
                        ...
                        while ($row = $query->fetch_assoc()) {
                            $daysDiff = dateDiffInDays2($row['fitness_due']);
                            ...
                            echo "<tr>
                                ...
                                <td" . (($daysDiff <= 15 && $daysDiff >= 0) ? " style='color:#ff5349 ; font-weight:bold'" : "") . ">" . $row['vehicle_class'] . "</td>
                                <td" . (($daysDiff <= 15 && $daysDiff >= 0) ? " style='color:#ff5349 ; font-weight:bold'" : "") . ">" . $row['body_type'] . "</td>
                                <td" . (($daysDiff <= 15 && $daysDiff >= 0) ? " style='color:#ff5349 ; font-weight:bold'" : "") . ">" . $row['fitness_due'] . "</td>
                                <td>
                                    <a href='#edit_" . $row['id'] . "' class='btn btn-success btn-sm' data-toggle='modal'><i class='fas fa-eye'></i>  View</a>
                                    <a href='#delete_" . $row['id'] . "' class='btn btn-danger btn-sm' data-toggle='modal'><span class='glyphicon glyphicon-trash'></span> Delete</a>
                            ...
                            include('./CRUD/edit_delete_modal.php');
							include('./CRUD/update_fitness.php');
                        }
                        ?>
							</tbody>
						</table>
			...
			<div class="height10"></div>
					...
					<div class="table-responsive">
						<table id="myTable3" class="table table-bordered table-striped">
							...
							function dateDiffInDays3($registration_date) {
								$currentTimestamp = strtotime('today');
								$expiryTimestamp = strtotime($registration_date);
								$diff = ($expiryTimestamp - $currentTimestamp) / (60 * 60 * 24);
								return floor($diff);
							}
							...
							while ($row = $query->fetch_assoc()) {
								$daysDiff = dateDiffInDays3($row['registration_date']);

								echo "<tr>
									<td>" . $row['id'] . "</td>
									<td" . (($daysDiff <= 365 && $daysDiff >= 0) ? " style='color:#ff5349 ; font-weight:bold'" : "") . ">" . $row['registration_no'] . "</td>
									<td" . (($daysDiff <= 365 && $daysDiff >= 0) ? " style='color:#ff5349 ; font-weight:bold'" : "") . ">" . $row['vehicle_class'] . "</td>
									<td" . (($daysDiff <= 365 && $daysDiff >= 0) ? " style='color:#ff5349 ; font-weight:bold'" : "") . ">" . $row['body_type'] . "</td>
									<td" . (($daysDiff <= 365 && $daysDiff >= 0) ? " style='color:#ff5349 ; font-weight:bold'" : "") . ">" . $row['registration_date'] . "</td>
									<td>
										<a href='#edit_" . $row['id'] . "' class='btn btn-success btn-sm' data-toggle='modal'><i class='fas fa-eye'></i>  View</a>
										<a href='#delete_" . $row['id'] . "' class='btn btn-danger btn-sm' data-toggle='modal'><span class='glyphicon glyphicon-trash'></span> Delete</a>
										<a href='#maintanence_" . $row['id'] . "' class='btn btn-primary btn-sm' data-toggle='modal'><span class='glyphicon glyphicon-share-alt'></span> Action</a>
									</td>
								</tr>";

								include('./CRUD/edit_delete_modal.php');
								include('./CRUD/update_maintanence.php');
							}
							?>
							</tbody>
						</table>
			...
			<div class="height10"></div>
					...
					<div class="table-responsive">
						<table id="myTable4" class="table table-bordered table-striped">
							...
							function dateDiffInDays4($registration_date) {
								$currentTimestamp = strtotime('today');
								$expiryTimestamp = strtotime($registration_date);
								$diff = ($expiryTimestamp - $currentTimestamp) / (60 * 60 * 24);
								return floor($diff);
							...
							while ($row = $query->fetch_assoc()) {
								$daysDiff = dateDiffInDays1($row['registration_date']);
								...
								echo "<tr>
									...
									<td" . (($daysDiff <= 15 && $daysDiff >= 0) ? " style='color:#ff5349 ; font-weight:bold'" : "") . ">" . $row['vehicle_class'] . "</td>
									<td" . (($daysDiff <= 15 && $daysDiff >= 0) ? " style='color:#ff5349 ; font-weight:bold'" : "") . ">" . $row['body_type'] . "</td>
									<td" . (($daysDiff <= 15 && $daysDiff >= 0) ? " style='color:#ff5349 ; font-weight:bold'" : "") . ">" . $row['registration_date'] . "</td>
									<td>
										<a href='#edit_" . $row['id'] . "' class='btn btn-success btn-sm' data-toggle='modal'><i class='fas fa-eye'></i>  View</a>
										<a href='#delete_" . $row['id'] . "' class='btn btn-danger btn-sm' data-toggle='modal'><span class='glyphicon glyphicon-trash'></span> Delete</a>
										<a href='#puc_" . $row['id'] . "' class='btn btn-primary btn-sm' data-toggle='modal'><span class='glyphicon glyphicon-share-alt'></span> Action</a>
	...
	<script>
		...
		function showContent(tabId) {
			document.querySelectorAll('.content').forEach(function (content) {
				...
				if ($.fn.DataTable.isDataTable('#myTable','#myTable1','#myTable2', content)) {
					$('#myTable','#myTable1','#myTable2', content).DataTable().destroy();
				}
			});
			...
			$('#myTable','#myTable1','#myTable2', document.getElementById('content-' + tabId)).DataTable(); 
		}	
